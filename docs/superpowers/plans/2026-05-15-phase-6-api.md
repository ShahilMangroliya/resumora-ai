...
- **`warmup_on_startup` defaults to `False`.** TestClient triggers the lifespan; we must not download a real DistilBERT during `pytest`. Production sets `RESUMEFIT_WARMUP_ON_STARTUP=true` so the model load failure surfaces at startup, not on the first request.
- **CORS is wired now.** Default origin `http://localhost:3000`; additional origins come from `RESUMEFIT_CORS_ORIGINS` (comma-separated). Adding it later means a debug round-trip mid-Phase-7.
...
git commit -m "feat: add settings loader for ResumeFit API"
```
...
The route is intentionally thin: parse the multipart upload, hand off to `run_analysis`. `IngestionError` is mapped to HTTP 400 via a FastAPI exception handler. CORSMiddleware allows the Phase 7 frontend. The lifespan triggers singleton warm-up only when `RESUMEFIT_WARMUP_ON_STARTUP=true`.
...
- [ ] `RESUMEFIT_WARMUP_ON_STARTUP=false` is the default — TestClient does not attempt a real model download.
